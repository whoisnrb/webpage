...
import { Database, ShieldCheck, Server, Cloud, CheckCircle2, ArrowRight, Sparkles } from "lucide-react"
...
export function BackupContent() {
    const t = useTranslations("Services.BackupDataRecovery")
    ...
    return (
        <div className="flex flex-col min-h-screen bg-transparent">
            <ServiceLayout
                ...
                techStack={["Synology DSM", "Veeam Backup", "Acronis", "AWS S3", "Backblaze B2", "Restic", "Cryptography"]}
                pricing={tServices('item_labels.pricing_from_format', {
                    price: getPriceString(10000)
                })}
            >
                <UseCases
                                ...
                                >
                                    ...
                                    <SpotlightCard className={`h-full bg-white/[0.02] border-white/10 rounded-[48px] p-10 flex flex-col ${plan.popular ? 'border-primary/50 bg-primary/[0.02]' : ''}`}>
                                            ...
                                            <div className="flex items-baseline gap-2">
                                                <div className="text-6xl font-black text-white tracking-tighter">
                                                    {plan.priceText ? plan.priceText : plan.priceFrom ? (
                                                        ...
                                                        }) + tServices('item_labels.pricing_per_month')
                                                    ) : (
                                        ...
                                        <div className="pt-8 border-t border-white/5 mt-auto">
                                            ...
                                            {plan.price ? (
                                                <CheckoutButton 
                                                    serviceName={`Backup & Mentési Stratégia - ${plan.name}`}
                                                    price={plan.price}
                                                    currency="HUF"
                                                ...
                                                <Button className={`w-full h-16 rounded-2xl text-lg font-black uppercase tracking-tight transition-all duration-500 bg-white/[0.05] hover:bg-white/[0.1] text-white border border-white/10`} asChild>
                                                    <Link href={{ pathname: "/ajanlatkeres", query: { service: "Adatmentés & Backup" } }}>
                                                        {tServices("cta_quote")}
                                                        <ArrowRight className="ml-3 h-6 w-6" />
                                                    </Link>
                                                </Button>
