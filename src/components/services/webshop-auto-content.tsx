...
import { RefreshCw, Layout, Database, ShoppingBag, CheckCircle2, ArrowRight, Sparkles } from "lucide-react"
...
export function WebshopAutoContent() {
    const t = useTranslations("Services.WebshopAutomation")
    ...
    return (
        <div className="flex flex-col min-h-screen bg-transparent">
            <ServiceLayout
                ...
                icon={<RefreshCw className="h-24 w-24" />}
                features={[
                    t("hero_features.0"),
                    t("hero_features.1"),
                    t("hero_features.2"),
                ...
                techStack={["WooCommerce", "Shopify", "n8n", "Make", "Számlázz.hu", "Billingo", "GLS API", "DPD API", "Python", "Node.js"]}
                pricing={tServices('item_labels.pricing_from_format', {
                    price: getPriceString(120000)
                })}
            >
                <UseCases
                                ...
                                >
                                    ...
                                    <SpotlightCard className={`h-full bg-white/[0.02] border-white/10 rounded-[48px] p-10 flex flex-col ${plan.popular ? 'border-primary/50 bg-primary/[0.02]' : ''}`}>
                                        ...
                                        <div className="pt-8 border-t border-white/5 mt-auto">
                                            ...
                                            {plan.price ? (
                                                <CheckoutButton 
                                                    serviceName={`Webshop Automatizáció - ${plan.name}`}
                                                    price={plan.price}
                                                    currency="HUF"
                                                ...
                                                <Button className={`w-full h-16 rounded-2xl text-lg font-black uppercase tracking-tight transition-all duration-500 bg-white/[0.05] hover:bg-white/[0.1] text-white border border-white/10`} asChild>
                                                    <Link href={{ pathname: "/ajanlatkeres", query: { service: "Webshop Automatizáció" } }}>
                                                        {tServices("cta_quote")}
                                                        <ArrowRight className="ml-3 h-6 w-6" />
                                                    </Link>
                                                </Button>
